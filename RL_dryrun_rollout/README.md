...
这个模块只做 GRPO 训练前诊断，不训练模型。它会加载一个 student/base model 和可选 LoRA adapter，对 GSM8K train prompts 做多次采样，检查同一题的多条 completion 是否有正确/错误混合，从而判断是否有有效 reward variance，能否开始RL阶段
...
```text
outputs/qwen25_3b_base_sft_dryrun_g8_train500/
```

## Colab 运行

先在 Colab Secret 里配置需要的 token，例如 `HF_TOKEN`。这里用的是公开模型，也可以不配置
...
- `rollouts.jsonl`：每个 prompt 一行，包含多条采样结果、抽取答案、reward，这个人读起来比较困难，仅仅是备用分析使用
...
如果换成 base 模型且不想使用 chat template，将 `adapter_path` 留空，并设置：
