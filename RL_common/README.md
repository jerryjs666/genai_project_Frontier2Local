...
这是 GSM8K RL post-training各阶段的共享工具包，例如reward构成、评测、加载数据和模型等公共接口。不直接运行训练。

## 内容

- `answers.py`：答案抽取与归一化。优先 `The answer is <number>`，其次 `\boxed{...}`，最后 fallback 到最后一个数值表达式。
- `data.py`：加载 GSM8K train/test、现有 SFT train/val ID 文件，并映射官方 gold answer，简而言之就是validation与sft保持一致。
- `rewards.py`：answer reward、format reward、parse/length penalty、dry-run group diagnostics。
